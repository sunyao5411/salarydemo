...
(
   ...
   POSITION_NAME        varchar(64) COMMENT '职位名称' NOT NULL ,
...
(
   ...
   ADJUST               long COMMENT '加减项',
   PASS_BONUS           long COMMENT '放行费',
   COMPACT_SALARY       long COMMENT '合计',
   TEST_RESULT          int COMMENT '测评成绩',
   EXAMINE_BONUS        int COMMENT '考核奖',
   TOTAL_SALARY         long COMMENT '最终薪水',
   YEAR                 int COMMENT '年份',
...
(
   ...
   USER_NAME            varchar(64) COMMENT '登陆用户名' NOT NULL ,
   PASSWORD             varchar(256) COMMENT '用户密码' NOT NULL ,
   DISPLAY_NAME         varchar(128) COMMENT '用户姓名',
   POSITION_ID          varchar(64) COMMENT '职位ID',
   EMAIL                varchar(255) COMMENT '用户邮箱' NOT NULL ,
   DESCRIPTION          text COMMENT '描述',
   CREATE_TIME          datetime COMMENT '创建日期',
   CREATE_USER          varchar(64) COMMENT '创建人',
   UPDATE_TIME          datetime COMMENT '更新日期',
